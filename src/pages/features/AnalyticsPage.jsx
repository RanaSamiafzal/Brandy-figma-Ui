...
import { Shield, TrendingUp, BarChart3, PieChart, LineChart, Activity, Eye, ArrowRight } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-[#6b7280] max-w-3xl mx-auto mb-8">
              Make data-driven decisions with comprehensive analytics. Track performance, measure ROI, 
              and optimize your influencer marketing strategy with actionable insights.
            </p>
          ...
          <div className="relative max-w-5xl mx-auto">
            ...
            <div className="absolute -bottom-6 left-6 right-6 grid grid-cols-2 md:grid-cols-4 gap-4">
              {metrics.map((metric, index) => (
                <div key={index} className="bg-white p-4 rounded-xl shadow-lg border border-[#e5e7eb]">
                  <div className="text-sm text-[#6b7280] mb-1">{metric.label}</div>
                  <div className="text-2xl font-bold text-[#111827] mb-1">{metric.value}</div>
                  <div className="text-xs text-[#10b981] font-medium">{metric.change}</div>
                </div>
              ))}
            </div>
          </div>
