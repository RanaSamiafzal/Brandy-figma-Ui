...
import { Mail, Phone, MapPin, Send, MessageSquare, Shield, ArrowRight, Instagram, Twitter, Linkedin, Facebook } from 'lucide-react';
  ...
  const faqs = [
    {
      ...
      answer: "Simply create an account, complete your profile, and you can start searching for influencers or create your first campaign immediately.",
    },
    {
      question: 'Is there a cost for influencers to join?',
      answer: "Joining Brandly as an influencer is completely free. We take a small commission only when you successfully complete a collaboration.",
    },
    {
      question: 'How are payments handled?',
      answer: "We use a secure escrow system. Brands pay when the collaboration starts, and funds are released to influencers once the work is approved.",
    },
    {
      question: 'What if I need help with my campaign?',
      answer: "Our dedicated support team and campaign managers are available to assist you at every step of your influencer marketing journey.",
    },
  ];
    ...
    <div className="min-h-screen bg-white">
          ...
          <div className="flex flex-col lg:flex-row gap-12 items-center">
            <div className="w-full lg:w-1/2">
              ...
              <p className="text-xl text-[#6b7280] mb-8 leading-relaxed">
                Have questions about our platform or interested in working with us? 
                Our team is here to help you navigate the world of influencer marketing.
              </p>
              
              <div className="space-y-6">
                {contactMethods.map((method, index) => (
                  <div key={index} className="flex items-start gap-4 p-4 rounded-xl border border-[#e5e7eb] bg-white shadow-sm">
                    <div className={`w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${method.color}`}>
                      <method.icon className="w-6 h-6" />
                    </div>
                    <div>
                      <h3 className="font-bold text-[#111827]">{method.title}</h3>
                      <p className="text-sm text-[#6b7280] mb-1">{method.description}</p>
                      <p className="text-[#3b82f6] font-semibold">{method.value}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-10">
                <h3 className="text-sm font-bold text-[#111827] mb-4 uppercase tracking-wider">Follow Us</h3>
                <div className="flex gap-4">
                  {[Twitter, Linkedin, Instagram, Facebook].map((Icon, i) => (
                    <button key={i} className="w-10 h-10 rounded-lg border border-[#e5e7eb] flex items-center justify-center text-[#6b7280] hover:text-[#3b82f6] hover:border-[#3b82f6] transition-all">
            ...
            <div className="w-full lg:w-1/2">
              <div className="bg-white p-8 sm:p-10 rounded-3xl border border-[#e5e7eb] shadow-xl">
                ...
                <form className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium text-[#374151]">How can we help?</label>
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold text-[#111827] mb-6">Frequently Asked Questions</h2>
          ...
          <div className="space-y-6">
            {faqs.map((faq, index) => (
              <div key={index} className="p-6 rounded-2xl border border-[#e5e7eb] hover:border-[#3b82f6] transition-all group">
      ...
      <section className="py-20 bg-[#f9fafb] px-6">
        <div className="max-w-7xl mx-auto text-center">
          <h2 className="text-3xl font-bold text-[#111827] mb-12">Our Global Offices</h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-left">
            {[
              { city: 'San Francisco', address: '123 Innovation Way', state: 'CA 94103, USA' },
              { city: 'London', address: '45 Tech Plaza', state: 'EC2A 4NE, UK' },
              { city: 'Tokyo', address: '7-1 Digital Street', state: 'Minato-ku, Japan' },
            ].map((office, idx) => (
              <div key={idx} className="bg-white p-6 rounded-2xl border border-[#e5e7eb]">
                <h3 className="text-xl font-bold text-[#111827] mb-3">{office.city}</h3>
